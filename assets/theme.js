...
$(document).ready(function() {
  ...
  $(document).ready(function(){
    $('[data-bundle-atc]').on('click',function(e){
      e.preventDefault();
      let bundleName = $(this).parents('[data-section-id="bundle-collection-template"]').find('.section-header').attr('data-collection');
      $(this).parents('[data-section-id="bundle-collection-template"]').find('[data-collection-product]').each(function(){
        var data_ajax = {
          quantity:1,
          id: $(this).val(),
          properties: {
        ...
        var params = {
          ...
          success: function(line_item) {
            ...
            $.getJSON('/cart.js', function(cart) {  // Update Cart Item count 
...
//  get bundle and wrapper class
  let bundleNames = [];
  $('.bundle__product').each(function(){
    let bundleName = $(this).attr('data-bundle-name');
    bundleNames.push(bundleName);
  })
  let uniqueBundleNames = $.unique(bundleNames);
  $.each(uniqueBundleNames, function(i, bundleName){
    $('[data-bundle-name="'+bundleName+'"]').wrapAll('<div class="bundle__wrapper" data-bundle-name="'+bundleName+'"></div>')
  });
  setTimeout(function(){ 
    $('.bundle__wrapper').each(function(){
      $(this).prepend('<div class="remove__bundle"><a href="#" data-remove-bundle class="btn">Remove</a></div>');
      $(this).prepend('<div class="bundle__name h1">'+$(this).attr("data-bundle-name")+'</div>');
    })

    $('[data-remove-bundle]').on('click',function(e){
      e.preventDefault();
      console.log('clicked');
      let lineIds = [];
      $(this).parents('.bundle__wrapper').find('.bundle__product').each(function(){
        lineIds.push($(this).attr('data-line'));
      })
...
// CART item qty changes 
  $('[data-cart-product-qty]').on('change', function(){
    let qty = $(this).val();
    
    if($(this).parents('.bundle__product').length > 0){
      
      let itemIds = [];
      $(this).parents('.bundle__wrapper').find('.bundle__product').each(function(){
      	itemIds.push($(this).find('[data-cart-product-qty]').attr('data-item-id'));
      })
      console.log(itemIds);
      var updateCartData = "";
      $.each(itemIds,function(i, itemId){
        updateCartData += 'updates['+itemId+']='+qty+'&';   
      })
          ...
          if(cart.items.length > 0){
            $.each(cart.items, function( index, item ) {    
              ...
              $('.cart__row').each(function(){
                if($(this).find('[data-cart-product-qty]').attr('data-item-id') == item.id){
                  $(this).find('[data-cart-product-qty]').parents('.cart__row').find('.cart__price').html(Shopify.formatMoney(item.line_price, ""));
                  $(this).find('[data-cart-product-qty]').val(item.quantity);
            ...
            var updateCartData = 'updates['+itemId+']='+qty;
          	$.post('/cart/update.js',
             updateCartData
            );
            setTimeout(function(){ 
